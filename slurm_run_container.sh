...
mkdir -p "$RESULT_DIR/logs" "$RESULT_DIR/outputs/checkpoints" "$RESULT_DIR/outputs/embeddings" "$RESULT_DIR/outputs/results"

TMP_CONFIG="$RUN_DIR/config_job.yaml"
python - "$ROOT_DIR/$CONFIG_PATH" "$TMP_CONFIG" "$RESULT_DIR/logs" "$RESULT_DIR/outputs" <<'PY'
import sys
import yaml

src, dst, logs_dir, outputs_dir = sys.argv[1:5]
...
env \
	...
	apptainer exec \
		...
		python "$ROOT_DIR/main.py" --config "$TMP_CONFIG"
