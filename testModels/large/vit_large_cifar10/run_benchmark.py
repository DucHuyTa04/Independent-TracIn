"""Benchmark Ghost+FAISS vs Original TracIn (ViT-Large, CIFAR-10, ~22M params)."""
...
from testModels.large.vit_large_cifar10.data import Cifar10Dataset
from testModels.large.vit_large_cifar10.model import build_large_vit
...
class IndexListDataset(Dataset):
    def __init__(self, ds: Cifar10Dataset, positions: List[int]) -> None:
        self.ds = ds
...
def pick_subset_indices(ds: Cifar10Dataset, per_class: int, seed: int) -> List[int]:
...
def main() -> None:
    ...
    p.add_argument("--output-dir", default="outputs/benchmarks/vit_large_cifar10")
    ...
    full_train = Cifar10Dataset(train=True, root=args.data_root)
    ...
    ghost_layers = auto_ghost_layers(mod_g, target_coverage=1.0, max_layers=128)

    test_ds = Cifar10Dataset(train=False, root=args.data_root)
    ...
    mf_layers = auto_ghost_layers(mf_info, target_coverage=1.0, max_layers=128)
    ...
    plot_model_benchmark(
        "vit_large_cifar10", ghost_cmp, original_scores, out, sample_ids=compare_ids,
    )
