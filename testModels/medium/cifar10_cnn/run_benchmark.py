"""Benchmark Ghost+FAISS vs Original TracIn (CIFAR-10 subset)."""
...
def _label_to_int(y: torch.Tensor | int) -> int:
    return int(y) if not isinstance(y, torch.Tensor) else int(y.item())


class IndexListDataset(Dataset):
    def __init__(self, cifar_train: CifarDataset, positions: List[int]) -> None:
        self.ds = cifar_train
...
def main() -> None:
    ...
    p.add_argument("--output-dir", default="outputs/benchmarks/cifar10_cnn")
    ...
    def model_factory() -> CifarSmallCNN:
        torch.manual_seed(args.seed)
    ...
    metrics["model_info"] = model_ghost_coverage(mf_info, mf_layers[0], ghost_layers=mf_layers)
    ...
    plot_paths = plot_model_benchmark(
        "cifar10_cnn", ghost_cmp, original_scores, out, sample_ids=compare_ids,
    )
    if args.diagnostic:
        plot_paths.append(
            plot_diagnostic_variants(
                "cifar10_cnn", spearman_diag, out, variant_order=VARIANT_ORDER
            )
        )
