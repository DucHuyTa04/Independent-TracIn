"""Benchmark Ghost+FAISS vs Original TracIn (Encoder Transformer, TinyShakespeare classification)."""
...
from testModels.medium.encoder_transformer.data import CharClassifyDataset
from testModels.medium.encoder_transformer.model import TinyBERT


class ReindexedSubset(Dataset):
    def __init__(self, ds: CharClassifyDataset, indices: List[int]) -> None:
        self.ds = ds
...
def main() -> None:
    ...
    p.add_argument("--output-dir", default="outputs/benchmarks/encoder_transformer")
    ...
    # Data
    full_train = CharClassifyDataset(root=args.data_root, train=True)
    ...
    def model_factory() -> TinyBERT:
        torch.manual_seed(args.seed)
        return TinyBERT(vocab_size=vocab_size, num_classes=2)
    ...
    test_ds = CharClassifyDataset(root=args.data_root, train=False)
    ...
    plot_paths = plot_model_benchmark(
        "encoder_transformer", ghost_cmp, original_scores, out, sample_ids=compare_ids,
    )
    if args.diagnostic:
        plot_paths.append(
            plot_diagnostic_variants(
                "encoder_transformer", spearman_diag, out, variant_order=VARIANT_ORDER
            )
        )
