"""Benchmark Ghost+FAISS vs Original TracIn (ResNet-small, CIFAR-100)."""
...
from testModels.medium.resnet_cifar100.model import ResNetSmall
...
class IndexListDataset(Dataset):
    def __init__(self, ds: Cifar100Dataset, positions: List[int]) -> None:
        self.ds = ds
...
def pick_subset_indices(ds: Cifar100Dataset, per_class: int, seed: int) -> List[int]:
    torch.manual_seed(seed)
    by_c: dict[int, List[int]] = {c: [] for c in range(100)}
...
def main() -> None:
    ...
    p.add_argument("--output-dir", default="outputs/benchmarks/resnet_cifar100")
    ...
    full_train = Cifar100Dataset(train=True, root=args.data_root)
    ...
    sample_meta = {
        i: f"class_{_label_to_int(full_train[positions[i]][1])}" for i in range(n_train)
    }

    def model_factory() -> ResNetSmall:
        torch.manual_seed(args.seed)
        return ResNetSmall(num_classes=100)
    ...
    test_ds = Cifar100Dataset(train=False, root=args.data_root)
    ...
    # With Conv2d ghost support, hook both conv and FC layers for much higher
    # coverage.  "last" strategy picks FC head first, then deeper conv layers.
    ...
    plot_paths = plot_model_benchmark(
        "resnet_cifar100", ghost_cmp, original_scores, out, sample_ids=compare_ids,
    )
    if args.diagnostic:
        plot_paths.append(
            plot_diagnostic_variants(
                "resnet_cifar100", spearman_diag, out, variant_order=VARIANT_ORDER
            )
        )
