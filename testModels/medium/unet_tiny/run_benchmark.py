"""Benchmark Ghost+FAISS vs Original TracIn (tiny U-Net denoising on CIFAR-10)."""
...
from testModels.medium.unet_tiny.data import CifarDenoiseDataset
from testModels.medium.unet_tiny.model import TinyDenoiseUNet


class SubsetDataset(Dataset):
    def __init__(self, ds: CifarDenoiseDataset, indices: List[int]) -> None:
        self.ds = ds
    ...
    def __getitem__(self, i: int):
        real = self.indices[i]
        noisy, clean, _ = self.ds[real]
        return noisy, clean, i


def main() -> None:
    p = argparse.ArgumentParser()
    p.add_argument("--output-dir", default="outputs/benchmarks/unet_tiny")
    ...
    full_train = CifarDenoiseDataset(train=True, root=args.data_root)
    ...
    def model_factory() -> TinyDenoiseUNet:
        torch.manual_seed(args.seed)
        return TinyDenoiseUNet()
    ...
    for e in range(args.epochs):
        for noisy, clean, _ in dl_full:
            noisy, clean = noisy.to(device), clean.to(device)
            opt.zero_grad()
            crit(m(noisy), clean).backward()
            opt.step()
        if e in save_at:
    ...
    test_ds = CifarDenoiseDataset(train=False, root=args.data_root)
    ...
    plot_paths = plot_model_benchmark(
        "unet_tiny", ghost_cmp, original_scores, out, sample_ids=compare_ids,
    )
