"""Benchmark Ghost+FAISS vs Original TracIn (MNIST subset)."""
...
class IndexListDataset(Dataset):
    """Maps position i -> MNIST train image at underlying index positions[i]."""
...
def main() -> None:
    p = argparse.ArgumentParser(description="MNIST benchmark (Ghost+FAISS vs Original TracIn)")
    p.add_argument("--output-dir", default="outputs/benchmarks/mnist")
    p.add_argument("--per-class", type=int, default=1000, help="training samples per digit")
    ...
    p.add_argument(
        ...
        type=int,
        default=16,
        help="Number of test points (class 0) to average for query loss / TracIn",
    )
    p.add_argument(
        "--diagnostic",
        action="store_true",
        help="Also run variants B–E and save diagnostic_variants.png.",
    )
    p.add_argument(
        "--max-baseline-n",
        type=int,
    ...
    sample_meta = {i: f"digit_{int(mnist_full[positions[i]][1].item())}" for i in range(n_train)}

    def model_factory() -> MnistMLP:
        torch.manual_seed(args.seed)
        return MnistMLP()
    ...
    if args.diagnostic:
        ...
        variant_scores.update(mid)

    metrics = summarize_metrics(ghost_cmp, original_scores, compare_ids)
    metrics["n_train"] = n_train
    mf_info = model_factory()
    metrics["model_info"] = model_ghost_coverage(mf_info, mf_info.fc2, ghost_layers=[mf_info.fc1, mf_info.fc2])
    ...
    plot_paths = plot_model_benchmark(
        "mnist", ghost_cmp, original_scores, out, sample_ids=compare_ids,
    )
    if args.diagnostic:
        plot_paths.append(
            plot_diagnostic_variants(
                "mnist", spearman_diag, out, variant_order=VARIANT_ORDER
            )
        )
